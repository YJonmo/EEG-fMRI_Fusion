...
%% EEG EEG EEG EEG EEG EEG EEG EEG EEG EEG EEG EEG EEG EEG EEG EEG EEG EEG EEG EEG EEG EEG EEG EEG EEG EEG EEG EEG EEG EEG EEG EEG EEG EEG 
    ...
    if FileName == 0
        ...
        load BrainCap1010
        cfg.elec = elec;
        ForwardModel = Create_TemplateBEM(cfg, Data);
    ...
    %% Creating the source space data
    Beamed = data_short_resamp ;
    Beamed.trial{1} = SourceSpaceStuff.filters*data_short_resamp.trial{1} ;
    for II = 1:size(Beamed.trial{1},1)
        Beamed.label{II} = strcat('Sig',num2str(II));
    end    
    %% Band passs the Source space data
    cfg = [] ; 
    cfg.bpfilter = 'yes'
    EEGBandPass = [ 3.5  5; 5.1 8; 8 13 ] 
    for Band_Index = 1:size(EEGBandPass,1)
        cfg.bpfreq   = EEGBandPass(Band_Index,:)  
        BeamedBPed{Band_Index}= ft_preprocessing(cfg, Beamed) ;
        for I = 3:3:size(BeamedBPed{Band_Index}.trial{1},1)
            BeamedBPed{Band_Index}.trial2{1}(I/3,:) = abs(BeamedBPed{Band_Index}.trial{1}(I-2,:))+abs(BeamedBPed{Band_Index}.trial{1}(I-1,:))+abs(BeamedBPed{Band_Index}.trial{1}(I,:)) ;
        end
        BeamedBPed{Band_Index}.trial{1} = BeamedBPed{Band_Index}.trial2{1} ;

        BeamedBPed{Band_Index} = rmfield(BeamedBPed{Band_Index}, 'label') ;
        for II = 1:size(BeamedBPed{Band_Index}.trial{1},1)
            BeamedBPed{Band_Index}.label{II} = strcat('Sig',num2str(II));
        end
    end
    %%
    for Band_Index = 1:size(EEGBandPass,1)
    ...
    %     title('Quick linear interpolation of missing trial');
        ...
        SourceSpaceERF{Band_Index} = ft_resampledata(cfg, filtconv2)
    ...
    %    EEG_total.Spatial = [EEG_data.Spatial EEG_data.Spatial EEG_data.Spatial];% EEG_data.Spatial];% EEG_data.Spatial];% EEG_data.Spatial] ; 

    %     for I = 1:size(EEG_total.trial{1},1)
    %         EEG_total.label{I} = strcat('Sig',num2str(I));
    %     end
    %     
        SourceSpaceERF{Band_Index} = EEG_total ; 
    end

    %% Creating the total HRFed data
    No_Vox = size(EEG_total.trial{1},1) ;
    SourceSpaceERF_total = SourceSpaceERF{1} ; 
    SourceSpaceERF_total.trial{1} = zeros(Size_temp*size(EEGBandPass,1) , size(SourceSpaceERF{1}.trial{1},2)) ; 
    for Band_Index = 1:size(EEGBandPass,1)
        SourceSpaceERF_total.trial{1}(1 + (Band_Index - 1)*No_Vox:No_Vox*Band_Index, :) = SourceSpaceERF{Band_Index}.trial{1} ;
    end
    for I = 1:size(SourceSpaceERF_total.trial{1},1)
        SourceSpaceERF_total.label{I} = strcat('HRF',num2str(I));
    end
    ...
    paradigm3.label{2} = 'Pardig2' ;
    paradigm3.label{3} = 'Pardig3' ;

    %% Reduce the size of the data using SVD
    cfg_Mul = [] ; 
    cfg_Mul.bandpass = [ .01  0.02] ;% 0.02  0.065; 0.06  0.11; ]       % Defining the filter bands in Hz
    cfg_Mul.bandRanks = [10];% 10; 10] ;                              % Manually define how many principal components from each band should be inlcuded 
%    cfg_Mul.pca = 'svd' ;%[] ;                                                   % or use 'svd' for the purpuse of artifact removal.
    % Choosing the ICA algorithm 
    % SOBI gives the best decomposition with the multi-ICA algorithm
    cfg_Mul.method = 'svd' ; % Other methods 'runica', 'fastica', 'binica', 'pca', 'svd', 'jader', 'varimax', 'dss', 'cca', 'sobi', 'white'
    cfg_Mul.pca = 'ft_svd' ;
    cfg_Mul.elec.label = SourceSpaceERF_total.label ;
    [Multi_ICA_EEG] = MultiBand_ICA_Simplified(cfg_Mul, SourceSpaceERF_total);
       
    %% Correct the spatial maps of the ICs based on the number of the bands
    Multi_ICA_EEG.SpatialICs = Multi_ICA_EEG.SpatialICs ; 
%     for I = 1:size(Multi_ICA_EEG.SpatialICs,2)
%         Multi_ICA_EEG.Spatial(:,I) = 
%         Multi_ICA_EEG.SpatialPCs =
%     end
%     
    for Comp_Index = 1:size(Multi_ICA_EEG.SpatialICs,2)
        Multi_ICA_EEG.SpatialICs(:,Comp_Index) = Multi_ICA_EEG.SpatialICs(:,Comp_Index)/max(abs(Multi_ICA_EEG.SpatialICs(:,Comp_Index))) ;
    end 
...
%% Fusion Fusion Fusion Fusion Fusion Fusion Fusion Fusion Fusion Fusion Fusion Fusion Fusion Fusion Fusion Fusion Fusion Fusion Fusion Fusion Fusion Fusion 
    ...
    cfg_fus = [] ; 
    cfg_fus.method = 'sobi' ;    % Other methods 'runica', 'fastica', 'binica', 'pca', 'svd', 'jader', 'varimax', 'dss', 'cca', 'sobi', 'white'
    ...
    for ii = 1:Band_Index
        simple3Dmapping(Multi_ICA_EEG.SpatialICs(1 + (ii-1)*4345:ii*4345,Curren_comp), ForwardModel.grid.pos(ForwardModel.grid.inside,:))
        %simple3Dmapping(FusedStuff.SpatialICs_EEG(1 + (ii-1)*4345:ii*4345,Curren_comp), ForwardModel.grid.pos(ForwardModel.grid.inside,:))
        title(strcat(['Component ', num2str(Curren_comp) 'band', num2str(ii)]))
    end
...
%     cfg = [] ;
%     cfg.Comps = [21] ; 
%     SourceICA_plotmaps(cfg, SourceSpaceStuff2, ForwardModel)
    
    %% Map on the MRI image
    Curren_comp = 1 ; 

    for ii = 2:2
        Map_Temp = Multi_ICA_EEG.SpatialICs(1 + (ii-1)*4345:ii*4345,Curren_comp) ;
        Inside_count = 0;
        source = ForwardModel.grid;
        for Vox_Index = 1 : size(ForwardModel.grid.inside,1)
            if  ForwardModel.grid.inside(Vox_Index) == 1
                Inside_count = Inside_count + 1;
                source.avg.Map(Vox_Index) = Map_Temp(Inside_count,1) ;
            else
    ...
    cfg.filename = strcat(['Comp_', num2str(Curren_comp), '_Band_', num2str(ii),  '_PLA_NBk_018.nii']);
